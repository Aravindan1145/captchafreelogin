...
export const generateSeatDetails = (passengerCount: number) => {
  const coaches = ['S1', 'S2', 'S3', 'A1', 'A2', 'B1'];
  const berths = ['Lower Berth', 'Middle Berth', 'Upper Berth', 'Side Lower', 'Side Upper'];
  
  return Array.from({ length: passengerCount }, (_, i) => ({
    coach: coaches[Math.floor(Math.random() * coaches.length)],
    seatNumber: (Math.floor(Math.random() * 72) + 1).toString(),
